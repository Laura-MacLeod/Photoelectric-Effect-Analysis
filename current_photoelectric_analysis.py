...
# -*- coding: utf-8 -*-
"""
Created on Fri Oct 28 08:29:07 2022
...
def Growth(x, a, b, c, d):
    out = a + b/(np.exp(c+ d*x))
    return out
...
blue_verr_half = 0.04*blue_v_half
...
cut_blue_verr_half = blue_verr_half.iloc[cut_blue_v_half.index[0]:]
...
blue_range = np.linspace(cut_blue_v_half[-1:], cut_blue_v_half[:1], 1000)
...
blue_zero = (intersect(physical(blue_range, para3[0], para3[1], para3[2], para3[3]), blue_range))
...
blue_cutoff_half = abs(intersect(physical(blue_range, para3_half[0], para3_half[1], para3_half[2], para3_half[3]),blue_range))
